...
[CRITICAL FIX]
- ACTION MASK (STRICT): allow ONLY tokens whose decoded text consists of:
    - ASCII digits: '0'..'9'
    - ASCII space:  ' '
  plus EOS token id explicitly.
  This prevents reward-hacking via Unicode digits, tabs/newlines, full-width spaces, superscripts, etc.

[CRITICAL FIX]
- Node parser (STRICT): accept ONLY ASCII digits and ASCII spaces. Any other char => invalid_token and terminate.

- reward_miss_target: penalize ending episode without success to prevent "immediate stop" collapse.
- TD target max is computed WITH THE SAME action mask (otherwise illegal-token Q can dominate).
...
def select_next_token(
    ...
    # SAFETY: do NOT fall back to unmasked logits (that would bypass the mask).
    ...
    if epsilon > 0.0 and random.random() < epsilon:
        ...
        # idx should already be within allowed tokens because disallowed are -inf
...
def _is_allowed_piece_ascii_digits_space(piece: str) -> bool:
    # Allow ONLY ASCII digits and ASCII spaces.
    # Reject any other whitespace (\n, \t, \r, full-width space, etc.) and any Unicode digit.
...
def build_allowed_token_mask_ascii_digits_space(
    tokenizer,
    device: torch.device,
    allow_eos: bool = True,
) -> Tensor:
    """
    Allowed tokens:
      - tokenizer.decode([tid]) consists ONLY of ASCII digits '0'..'9' and ASCII space ' '
      - plus EOS token id explicitly (if allow_eos=True)
    """
    V = len(tokenizer)
    ...
    for tid in range(V):
        ...
        if _is_allowed_piece_ascii_digits_space(s):
            allowed[tid] = True
...
# -------------------------
# streaming node parser (STRICT ASCII)
# -------------------------
@dataclass
class ParseResult:
...
class NodeStreamParser:
    ...
    def _flush_pending(self) -> Optional[int]:
        ...
        # s is guaranteed ASCII digits only
        try:
            return int(s)
        except Exception:
            return None
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Q-learning (HF/Qwen) for composition graphs (K stages) FINAL (STRICT ASCII MASK)")
    ...
    p.add_argument(
        "--action_mask",
        type=str,
        default="ascii_digits_space",
        choices=["none", "ascii_digits_space"],
        help="ascii_digits_space: allow ONLY ASCII digits and ASCII space plus EOS. Recommended.",
    )
...
def main() -> None:
    ...
    if args.action_mask == "ascii_digits_space":
        t0 = time.perf_counter()
        allowed_token_mask = build_allowed_token_mask_ascii_digits_space(tokenizer, device=device, allow_eos=True)
        kept = int(allowed_token_mask.sum().item())
        print(f"[action_mask] ascii_digits_space enabled. allowed={kept}/{len(tokenizer)} tokens. build_sec={time.perf_counter()-t0:.2f}")
    else:
