...
v4 (stability + action-space alignment) key upgrades:
A) Stability (prevents collapse):
   1) TD loss scaled by --td_coef
   2) TD loss uses Huber (smooth_l1) by default
   3) Soft backup (logsumexp) option to reduce overestimation vs raw max
   4) Q baseline using ref logits: Q := logit_pi - logit_ref  (learn residual over SFT/ref)

B) Action space alignment (big win):
   5) Dynamic successor-prefix constrained decoding (requires ascii_digits_space_single):
      - Only allow digits that keep pending number as a prefix of some successor node id of CURRENT env node
      - Only allow ' ' (space) when pending forms an EXACT successor (commits a node)
      - Greatly reduces invalid transitions -> makes applied-path training tractable

C) Intermediate rewards (optional, "give more intermediate rewards"):
   6) Potential-based distance shaping to target:
      r += alpha * (dist(cur)->target - dist(next)->target), clipped

D) Step penalty semantics fix:
   7) step_penalty is charged per NODE attempt (when a node is committed), not per token.

Notes:
- Keep --action_mask ascii_digits_space_single. (You already validated it gives allowed=12 tokens.)
- This script is still token-level RL, but with successor-prefix guard it behaves close to node-level actions.
"""
...
def precompute_all_target_dists(
...
def _allowed_digits_by_successor_prefix(pending: str, succ_strs: Set[str]) -> Tuple[Set[str], bool]:
    """
    Returns:
      next_digits: set of digit chars allowed to extend pending while staying a prefix of some successor id
      allow_space: whether pending is already an exact successor id (can commit with space)
    """
    if not succ_strs:
        return set(), False
    ...
    # If pending empty, any successor is a "match"
...
def dynamic_candidate_token_ids(
    ...
    """
    Candidate ids for next token based on:
      - ascii_digits_space_single action space
      - optional successor-prefix constraint (recommended)
      - optional numeric range digit-guard (recommended)
    """
    if action_space is None:
        return None

    # If dynamic digit guard disabled AND successor guard disabled -> allow static
    ...
    # Successor prefix guard (strongly recommended)
    allow_space = False
    ...
    # Space policy:
    # - never allow space when pending is empty (avoid meaningless multiple spaces)
    # - if successor guard enabled: only allow space when pending is an exact successor
    # - else: allow space when pending != "" (and optionally allow_space_always)
    if pending != "":
        if args.dynamic_successor_prefix_guard:
            if allow_space:
...
def select_next_token_from_candidates(
    ...
    # epsilon: random among top-K candidates
...
# -------------------------
# Token-level trace: pending + env node state per timestep (for KL/TD masking)
# -------------------------
def compute_pending_and_env_states_for_actions(
    ...
    """
    Returns:
      pending_states[t] = pending digits BEFORE consuming actions[t]
      env_states[t]     = current env node BEFORE consuming actions[t]
    Both length T+1 (last is state after all actions).

    This is used to build candidate action sets consistent with rollout constraints,
    especially successor-prefix guard, which depends on current env node.
    """
    pending = ""
    cur = int(source)
    invalid_cnt = 0
    ...
    for aid in actions:
        ...
        # single-char digit / single space expected
        for ch in piece:
            if "0" <= ch <= "9":
                pending += ch
            elif ch == " ":
...
def sample_trajectory_hf(
    ...
    def apply_node_transition(next_node: int) -> Tuple[float, bool]:
        ...
        # step_penalty is per NODE attempt (not per token)
        ...
        if int(next_node) in adj.get(int(current_node), set()):
            ...
            # stage bridge shaping (intermediate rewards)
            ...
            if int(next_node) == int(target):
                if args.reward_hit_target_requires_coverage and (not _covered_all_required()):
                    # reached target but missing required intermediate stages: terminate without hit reward
    ...
    for _step in range(max_new):
        ...
        if cand_ids is None:
            raise RuntimeError("ActionSpace is required for v4 (ascii_digits_space_single).")
    ...
    success = is_valid_path_k(path_for_success, int(source), int(target), node_to_stage=node_to_stage, adj=adj)

    # terminal miss penalty (process mode)
...
# -------------------------
# Losses (TD + KL) computed from logits to avoid extra forward passes
# -------------------------
def soft_backup_value(qvals: Tensor, mode: str, tau: float) -> Tensor:
    """
    qvals: [A] (candidate actions)
    returns scalar V
    """
    if mode == "max":
        return qvals.max()
...
def td_loss_from_logits(
    ...
    """
    TD over token actions, but Q is optionally residual over ref logits:
      Q := logit_pi - logit_ref
    Candidate sets depend on (pending, current_node) if successor-prefix guard enabled.
    """
    if len(actions) == 0:
    ...
    # build candidate ids per timestep (before each action)
...
def kl_loss_masked_full_dynamic_from_logits(
    ...
    """
    Exact KL(pi||ref) over the dynamic candidate action set at each timestep.
    Candidate sets depend on env state if successor-prefix guard enabled.
    """
    eos_id = int(tokenizer.eos_token_id)
...
def load_policy_model_and_ref(
    ...
    if args.kl_coef > 0.0 or args.q_baseline == "ref":
        # We need ref logits for KL and/or Q baseline
        if has_adapter:
            base2 = _load_base()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Q-learning (HF/Qwen) for composition graphs (K stages) v4 stable")
    ...
    p.add_argument("--rollout_temp_start", type=float, default=0.8)
    p.add_argument("--rollout_temp_end", type=float, default=0.3)
    p.add_argument("--temp_warmup_iters", type=int, default=2000)
    ...
    p.add_argument("--dynamic_successor_prefix_guard", action=argparse.BooleanOptionalAction, default=True)
    ...
    p.add_argument("--reward_hit_target", type=float, default=2.0)
    ...
    p.add_argument("--reward_valid_transition", type=float, default=0.05)
    p.add_argument("--reward_stage_bridge", type=float, default=0.25)
    p.add_argument("--reward_stage_bridge_only_once", action="store_true")

    p.add_argument("--reward_invalid_transition", type=float, default=0.8)
    p.add_argument("--reward_invalid_token", type=float, default=3.0)
    p.add_argument("--reward_stop", type=float, default=-0.1)
    p.add_argument("--reward_miss_target", type=float, default=2.0)
    ...
    # IMPORTANT: step_penalty is per NODE attempt now
    ...
    # KL
    p.add_argument("--kl_coef", type=float, default=0.15)
    p.add_argument("--kl_warmup_iters", type=int, default=0)
    p.add_argument("--kl_anneal_iters", type=int, default=0)
    p.add_argument("--kl_mode", choices=["masked_full_dynamic"], default="masked_full_dynamic")

    # TD / Q-learning stabilization
    p.add_argument("--td_coef", type=float, default=0.02, help="Scale TD loss. Keep small to avoid collapse.")
    p.add_argument("--td_loss", choices=["huber", "mse"], default="huber")
    p.add_argument("--td_backup", choices=["max", "logsumexp"], default="logsumexp")
    p.add_argument("--backup_tau", type=float, default=0.5, help="tau for logsumexp backup. Smaller -> closer to max.")
    p.add_argument("--q_baseline", choices=["none", "ref"], default="ref", help="Use Q := logit_pi - logit_ref baseline.")

    # distance shaping (intermediate rewards)
    p.add_argument("--reward_distance_shaping_alpha", type=float, default=0.15)
    p.add_argument("--distance_shaping_cap", type=float, default=0.3)
    ...
    p.add_argument("--log_dir", type=str, default="out_ql_hf_v4_stable")
...
def main() -> None:
    ...
    # precompute distances (intermediate reward shaping)
    dist_cache = None
    if args.reward_distance_shaping_alpha != 0.0:
    ...
    # action mask must be strict single-char for v4
    if args.action_mask != "ascii_digits_space_single":
        raise ValueError("v4 requires --action_mask ascii_digits_space_single")
    ...
    for iteration in range(1, args.max_iters + 1):
        ...
        for (s, t) in batch_pairs:
            ...
            logits_pi = forward_logits(policy_model, x_ids)[0]  # [L,V]
            logits_ref = None
            if ref_model is not None:
                with torch.no_grad():
