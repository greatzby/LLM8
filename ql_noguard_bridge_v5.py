...
v5 (NO-GUARD + bridge reward fix + stability fixes)

Key changes vs your v4:
1) NO-GUARD safety:
   - dynamic_digit_guard and dynamic_successor_prefix_guard default to False
   - forbid_any_guard=True by default (errors if you try to enable any guard)
   - errors if both guards are enabled (even if forbid_any_guard=False)

2) Bridge reward fix (IMPORTANT for P0 training):
   - Old: reward_stage_bridge only triggered when required_stages is non-empty -> P0 had required_stages=[]
   - New: reward_stage_bridge triggers on ANY VALID transition that advances exactly one stage:
         stage(next) == stage(cur) + 1
     So it actually provides signal in adjacent-only (P0) training.

3) Coverage tracking bugfix:
   - visited_required_stages is updated whenever entering a required intermediate stage,
     regardless of whether reward is set, so coverage gating works logically.

4) Stability:
   - Fix HF model loading arg: use torch_dtype (not dtype)
   - Add kl_min_coef (floor) to avoid KL annealing to 0 and collapsing
   - Cast KL computations to float32 for numerical stability

5) Logging:
   - report both success_applied and success_raw in training logs
   - eval reports used accuracy plus raw/applied accuracies
"""
...
def sample_trajectory_hf(
    ...
    def apply_node_transition(next_node: int) -> Tuple[float, bool]:
        ...
        # step_penalty is per NODE attempt
        ...
        if int(next_node) in adj.get(int(current_node), set()):
            ...
            # (A) Bridge reward FIX: reward stage +1 transitions (works in P0 training)
            ...
            # (C) Penalize overshooting target stage (irrecoverable in layered DAG)
            if (
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Q-learning (HF/Qwen) for composition graphs (K stages) v5 NO-GUARD")
    ...
    p.add_argument("--forbid_any_guard", action=argparse.BooleanOptionalAction, default=True,
                   help="If True, error if any guard is enabled (recommended for 'real ability' experiments).")
    ...
    # FIXED bridge reward: now for stage+1 transitions (works in P0)
    p.add_argument("--reward_stage_bridge", type=float, default=0.5)

    # optional extra reward for covering required intermediate stages (for long-pair training if you ever use it)
    ...
    p.add_argument("--penalty_target_stage_detour", type=float, default=0.2,
                   help="Penalty if model moves to a stage beyond target stage (overshoot).")
    ...
    p.add_argument("--kl_anneal_iters", type=int, default=0,
                   help="0 disables anneal (recommended).")
    p.add_argument("--kl_min_coef", type=float, default=0.05,
                   help="KL coefficient floor to prevent collapse when annealing. Set 0 to disable floor.")
    ...
    for iteration in range(1, args.max_iters + 1):
        ...
        if args.log_interval > 0 and (iteration % args.log_interval == 0):
            record = {
                ...
                "success_rate_applied": success_applied / len(batch_pairs),
