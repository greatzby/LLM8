...
def build_eval_buckets_kstages(
    ...
    Each test line is assumed: "source target ..." (we only use first 2 ints)
    """
    buckets: Dict[str, List[Tuple[int, int]]] = {f"S{i}->S{j}": [] for i in range(1, K + 1) for j in range(i + 1, K + 1)}
...
def evaluate_composition_qwen_batched_tokenparse_kstages(
    ...
    try:
        ...
        bucket_keys = sorted(buckets.keys(), key=lambda x: (int(x.split("->")[0][1:]), int(x.split("->")[1][1:])))
        ...
        for bkey in bucket_keys:
            ...
            for start in range(0, n, eval_gen_batch_size):
                ...
                for i_local, (source, target) in enumerate(batch_cases):
                    ...
                    # path tokens are expected after the "s t s " prompt
...
def main() -> None:
    ...
    for iter_num in range(args.max_iters + 1):
        ...
        if do_eval_now:
            ...
            # print all buckets in stage-order, plus overall
            keys = sorted([k for k in results.keys() if k != "overall"],
