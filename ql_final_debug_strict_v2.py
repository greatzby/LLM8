...
Fixes vs previous "STRICT ASCII" version:
1) ACTION MASK (STRICT-SINGLE-CHAR):
   Allow ONLY tokens whose decoded text is EXACTLY one of:
     - '0'..'9'
     - ' ' (single ASCII space)
   plus EOS token explicitly.
   This prevents multi-digit tokens like "1444" that let the model jump to out-of-range nodes cheaply.

2) NodeStreamParser (STRICT + EARLY CUT):
   - Accept ONLY ASCII digits and ASCII space
   - Enforce max digits based on node_max (e.g. node_max=149 => max_digits=3)
   - If digits exceed max_digits => invalid immediately
   - If digits reach max_digits and value > node_max => invalid immediately

3) Safer defaults:
   - max_rollout_steps=64 (but still capped by block_size)
   - learning_rate=1e-5
   - step_penalty=0.01
   - reward_invalid_token=3.0
"""
...
def select_next_token(
    ...
    if torch.isneginf(logits).all():
        # SAFETY: do NOT fall back to unmasked logits (bypass mask)
...
def _is_allowed_piece_ascii_digit_or_single_space(piece: str) -> bool:
    # STRICT-SINGLE-CHAR:
    # allow only exactly " " or exactly one digit "0".."9"
    if piece is None:
        return False
    if piece == " ":
        return True
...
def _is_allowed_piece_ascii_digits_space_loose(piece: str) -> bool:
    # LOOSER (legacy): allow pieces consisting only of digits/spaces, any length.
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Q-learning (HF/Qwen) for composition graphs (K stages) STRICT-SINGLE-CHAR MASK")
    ...
    p.add_argument("--log_dir", type=str, default="out_ql_hf_final_v2")
