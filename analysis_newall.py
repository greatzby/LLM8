...
# -*- coding: utf-8 -*-
"""
多任务阶段事件分析脚本（S1→S3 / S1→S2 / S2→S3, v5）

新增特性（相较 v4）：
1. --task-types 支持逗号分隔或 'all'，可一次跑多种任务；保留 --task-type 兼容旧调用。
2. 单次运行按任务分别解析样本、统计指标，同时复用同一模型推理循环。
3. 汇总输出在 CSV 中包含 task_type 字段，并为不同任务生成各自的 per-pair CSV（可选）。
4. 其他事件链条与统计逻辑沿用 v4（见脚本正文说明）。

原有 S1→S3 的 Event A/B/C/D、S1→S2 的 Event A′/B′/C′/D′、S2→S3 的 Event A/B/C 定义保持一致。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="(Deprecated) Single task type to run: s1s3, s1s2, or s2s3.",
    )
    parser.add_argument(
        "--task-types",
        type=str,
        default=None,
        help="Comma-separated list like 's1s3,s1s2' or use 'all' to run every task type.",
    )
...
def resolve_task_types(single: Optional[str], multi: Optional[str]) -> List[str]:
    ...
    for part in parts:
        if part not in VALID_TASK_TYPES:
            raise ValueError(f"Unsupported task type: {part}. Valid options: {', '.join(VALID_TASK_TYPES)} or 'all'.")
        if part not in seen:
            seen[part] = None
...
def aggregate_phase_events(records: List[PhaseEventRecord], task_type: str) -> Dict[str, float]:
    ...
    if task_type == "s1s3":
        ...
        eventB_prime_rate_given_A = (
            eventB_prime_total / eventA_prime_total if eventA_prime_total else 0.0
        )

        eventC_prime_total = sum(
            1 for r in records if r.legal_start and r.hit_stage2 and r.stage2_stop_token_emitted
        )
        eventC_prime_rate_given_AB = (
            eventC_prime_total / eventB_prime_total if eventB_prime_total else 0.0
        )

        eventD_prime_total = sum(1 for r in records if r.stage2_stop_success)
        eventD_prime_rate_given_ABC = (
            eventD_prime_total / eventC_prime_total if eventC_prime_total else 0.0
        )
        ...
        eventC_total = sum(
            1 for r in records if r.legal_start and r.hit_stage3 and r.stage3_stop_success
        )
...
def main() -> None:
    ...
    for task in task_list:
        pairs_raw = parse_test_pairs(data_dir / "test.txt", stages, task)
    ...
    tasks_to_run = [task for task in task_list if pairs_by_task.get(task)]
    tasks_without_data = [task for task in task_list if not pairs_by_task.get(task)]
    if tasks_without_data and not args.quiet:
        print(
            "[Warn] No pairs found for tasks: "
            + ", ".join(task.upper() for task in tasks_without_data)
        )

    if not tasks_to_run:
        raise RuntimeError("No task has available pairs to analyze. Please check your dataset filters.")

    if not args.quiet:
        msg = ", ".join(
            f"{task.upper()}: {len(pairs_by_task[task])} pairs" for task in tasks_to_run
        )
        print(f"Analyzing tasks -> {msg}")
    ...
    for step in iterator_steps:
        ...
        for task in tasks_to_run:
            ...
            if args.progress and not args.quiet:
                from tqdm import tqdm
                pair_iterator = enumerate(
                    tqdm(pairs, leave=False, desc=f"{task.upper()} Pairs@{step}")
                )
            ...
            for idx, info in pair_iterator:
                ...
                first_stage3_index = (
                    path_nodes.index(first_stage3) if hit_stage3 else None
                )
    ...
    if not args.quiet:
        ...
        if args.save_per_pair:
            print("Per-pair CSV files were generated per task and checkpoint.")
