...
v2_fix:
Fixes for on-policy correctness & stability:
1) IMPORTANT: disable dropout for PPO logprob recomputation (use model.eval() during teacher-forcing forward).
   - Rollout uses model.eval(); training forward must match it, otherwise PPO ratios are noisy/incorrect.

2) Sample prompts WITHOUT replacement by default (random.sample) to keep group sizes consistent.
   - random.choices() can repeat prompts -> group can become > group_size and distort normalization.

3) When forcing EOS at the end of rollout, compute EOS old_logprob properly instead of writing 0.0.

Everything else (env/reward/mask/parser) is kept identical to your v1.
"""
...
def sample_prompts(train_pairs: List[Pair], batch_size: int, with_replacement: bool) -> List[Pair]:
    ...
    # fallback (shouldn't happen in your setting)
    return random.choices(train_pairs, k=batch_size)
...
# -------------------------
def apply_node_reward_credit_to_digits(rewards: List[float], digit_step_indices: List[int], node_reward: float, mode: str) -> float:
    if mode == "none":
...
def dynamic_candidate_token_ids(
    ...
    # no guards: only format-only mask (space only allowed if pending != "")
    ...
    raise ValueError("This v2_fix keeps guards disabled. Please run with --no-dynamic_digit_guard --no-dynamic_successor_prefix_guard.")
...
def sample_trajectory_hf(
    ...
    # ensure EOS (FIX: compute EOS old_logprob instead of 0.0)
    if (not sampled_ids) or (int(sampled_ids[-1]) != int(eos_id)):
        # Compute logits for next token from full prefix (traj_ids currently does NOT include EOS)
...
def main() -> None:
    ...
    for iteration in range(1, args.max_iters + 1):
        ...
        # FIX: sample prompts (default without replacement)
        ...
        if args.log_interval > 0 and (iteration % args.log_interval == 0):
            record = {
                ...
                "success_rate_applied": success_applied / num_trajs,
