...
# -*- coding: utf-8 -*-
"""
Q-Learning fine-tuning script for GraphA datasets.

关键改动：
  1. rollout 温度 & ε-greedy 均支持分段调度（冷启动阶段几乎完全照抄 SFT）。
  2. 过程奖励新增正向 shaping：合法边奖励、中间 Stage 桥接奖励、停早惩罚等。
  ...
  7. 支持任意数量的 stage，不再硬编码 S1/S2/S3。
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--reward_stage_bridge", type=float, default=0.2,
                        help="Composition 任务首次触达中间 Stage 节点时的奖励。")
    parser.add_argument("--reward_stage_bridge_only_once", action="store_true",
                        help="达到中间 Stage 只奖励一次（默认 true）。")
...
def bucket_for_pair(source: int,
    ...
    """根据 node_to_stage 映射自动判定 pair 所属 bucket，如 'S1->S3'。"""
...
def is_valid_path(path_nodes: List[int],
    ...
    """验证路径合法性：边全部存在 + composition pair 经过所有中间 stage。"""
    if len(path_nodes) < 2:
        return False
