...
Key features:
  ...
  * Token-level loss reweighting for S1->S2 transitions via --s12_loss_multiplier.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--s12_loss_multiplier", type=float, default=4.0,
                        help="Multiplier applied to token-level cross-entropy when "
                             "current token is Stage1 and next token is Stage2. "
                             "Use 1.0 to disable reweighting.")
...
def compute_weighted_loss(
    ...
    s12_ids_stage1: torch.Tensor,
    s12_ids_stage2: torch.Tensor,
    multiplier: float,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    返回 (weighted_loss, base_loss)。
    logits: [B, T, V]
    targets / prev_tokens: [B, T]
    """
    B, T, V = logits.size()
    ...
    prev_flat = prev_tokens.view(-1)

    # mask: prev token 属于 Stage1 且 target token 属于 Stage2
    s1_mask = torch.isin(prev_flat, s12_ids_stage1)
    s2_mask = torch.isin(targets_flat, s12_ids_stage2)
    s12_mask = s1_mask & s2_mask  # bool

    if s12_mask.any():
        weights = torch.ones_like(ce)
        weights[s12_mask] *= multiplier
        weighted_loss = (ce * weights).mean()
    else:
...
def main() -> None:
    ...
    logger.info("S1->S2 loss multiplier: %.2f", args.s12_loss_multiplier)
    ...
    # 预先把 Stage1 / Stage2 的 token id 准备好
    try:
        s1_token_ids = torch.tensor(
            [stoi[str(node)] for node in stages[0]],
            dtype=torch.long,
            device=device,
        )
        s2_token_ids = torch.tensor(
            [stoi[str(node)] for node in stages[1]],
            dtype=torch.long,
            device=device,
        )
        ...
        missing = str(exc)
        raise KeyError(
            f"Stage node {missing} not found in vocabulary. "
            "Please confirm stage_info.pkl matches meta.pkl tokenization."
        ) from exc
    ...
    for iter_num in range(args.max_iters + 1):
        ...
        if iter_num % args.test_interval == 0:
            avg_train_loss_weighted = (
                running_weighted_loss / loss_counter if loss_counter > 0 else float("nan")
            )
            avg_train_loss_base = (
                running_base_loss / loss_counter if loss_counter > 0 else float("nan")
            )
            ...
            with torch.no_grad():
                for _ in range(10):
                    ...
                    weighted_loss, base_loss = compute_weighted_loss(
                        logits=logits_val,
                        targets=Y_val,
                        prev_tokens=X_val,
                        s12_ids_stage1=s1_token_ids,
                        s12_ids_stage2=s2_token_ids,
                        multiplier=args.s12_loss_multiplier,
                    )

                    val_weighted_losses.append(weighted_loss.item())
                    val_base_losses.append(base_loss.item())

                    B, T = Y_val.shape
                    prev_flat = X_val.view(-1)
                    target_flat = Y_val.view(-1)
                    s1_mask = torch.isin(prev_flat, s1_token_ids)
                    s2_mask = torch.isin(target_flat, s2_token_ids)
                    s12_mask = s1_mask & s2_mask
                    val_s12_tokens += int(s12_mask.sum().item())
                    val_total_tokens += B * T

            val_loss_weighted = float(np.mean(val_weighted_losses))
            val_loss_base = float(np.mean(val_base_losses))
            val_s12_ratio = (
                val_s12_tokens / val_total_tokens if val_total_tokens > 0 else 0.0
            )
            ...
            metrics_record = {
                "iter": iter_num,
                "lr": lr,
                "train_loss_weighted": avg_train_loss_weighted,
                "train_loss_base": avg_train_loss_base,
                "val_loss_weighted": val_loss_weighted,
                "val_loss_base": val_loss_base,
                "val_s12_token_ratio": val_s12_ratio,
                "accuracy": results,
            }
            ...
            logger.info(
                "Loss (weighted/base): train=%.4f/%.4f, val=%.4f/%.4f",
                avg_train_loss_weighted,
                avg_train_loss_base,
                val_loss_weighted,
                val_loss_base,
            )
            logger.info("  Val S1->S2 token ratio: %.2f%%", val_s12_ratio * 100.0)
