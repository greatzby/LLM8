...
v6 (NO-GUARD + node reward credit assignment + tiny format mask + eval_only)

Key changes vs v5:
1) Node-level reward credit assignment (A路线, no tokenizer change):
   ...
   - When a node transition reward is produced upon committing a node (SPACE/EOS),
     ...
     This reduces multi-token node-id credit assignment delay without any graph-guard.
...
# -------------------------
# node reward credit assignment (A路线)
# -------------------------
def apply_node_reward_credit_to_digits(
    ...
    if mode == "uniform_digits":
        if not digit_step_indices:
            # If there are no digits (should be rare), keep reward on delimiter to avoid dropping signal.
            return float(node_reward)
...
def dynamic_candidate_token_ids(
    ...
    if pending != "":
        if args.dynamic_successor_prefix_guard:
            ...
            # (Note: outer pending != "" already ensures we don't allow SPACE on empty pending)
            parts2.append(action_space.space_ids)
...
def sample_trajectory_hf(
    ...
    # A路线：记录“当前 pending node 的 digit token 步”的索引
    ...
    for _step in range(max_new):
        ...
        # A路线：记录 digit token 的步索引（用于后续把 node-level reward 回填给 digits）
        ...
        if int(next_id) == int(eos_id):
            for node in parser.finalize():
                ...
                # A路线：node-level reward 回填到 digits
...
def parse_args() -> argparse.Namespace:
    ...
    # A路线：node reward credit assignment
    p.add_argument(
        "--node_reward_credit",
        type=str,
        default="none",
