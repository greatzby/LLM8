...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Remove stage-skip pairs from training set; keep eval unchanged.")
    p.add_argument("--src-dir", type=Path, required=True, help="Full dataset directory (source).")
    p.add_argument("--dest-dir", type=Path, required=True, help="Destination directory (variant).")

    p.add_argument("--paths-per-pair", type=int, default=20, help="Used to locate train_{K}.txt.")
    p.add_argument("--train-file-pattern", type=str, default="train_{paths}.txt",
                   help="Format string for the training file.")

    p.add_argument("--stage-info-name", type=str, default="stage_info.pkl",
    ...
    p.add_argument("--target-index", type=int, default=1,
                   help="Token index for target node in each line (supports negative index). "
                        "NOTE: In your generator, format is: [src tgt ...path...], so default target-index=1.")

    p.add_argument("--max-jump", type=int, default=1,
                   help="Keep pairs only if (dst_stage - src_stage) <= max_jump. "
                        "For adjacent-only training, use 1.")
    p.add_argument("--keep-intra-stage", action="store_true",
                   help="Also keep pairs with (dst_stage - src_stage)==0 (intra-stage). Default: False.")
    p.add_argument("--keep-unknown-stage", action="store_true",
                   help="If a node is missing from stage_info, keep it (category=Unknown). Default: False (drop).")

    p.add_argument("--seed", type=int, default=2025, help="Shuffle seed for output training file.")
    p.add_argument("--pair-report", type=Path, default=None,
    ...
    p.add_argument(
        ...
        help="Files to copy from src to dest if they exist (test.txt will be copied unchanged).",
    )
    return p.parse_args()
...
def maybe_dump_pair_report(
    ...
    with open(report_path, "w", newline="", encoding="utf-8") as f:
        ...
        for (src, tgt), lines in sorted(pair_to_lines.items()):
            kept, ptype, jump = should_keep_pair(src, tgt, node_to_stage, max_jump, keep_intra, keep_unknown)
...
def main() -> None:
    ...
    for (src, tgt), lines in pair_to_lines.items():
        keep, ptype, jump = should_keep_pair(
            src, tgt,
    ...
    maybe_dump_pair_report(
        ...
        node_to_stage,
        args.max_jump,
        args.keep_intra_stage,
        args.keep_unknown_stage,
    )
    ...
    print("- KEPT")
    print(f"  pairs={kept_pairs_total:8d} | paths={kept_paths_total:8d} "
          f"| kept_paths_ratio={summary['ratios']['kept_paths_ratio']:.3f}")
    print(f"Rule: keep jump<= {args.max_jump} "
