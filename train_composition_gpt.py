...
Key features:
  ...
  * Stratified accuracy reporting for arbitrary stage groups plus overall.
...
def evaluate_composition(
    ...
    # Build node -> stage index mapping
    ...
    # Dynamically create buckets based on actual test data
    buckets: Dict[str, List[Tuple[int, int, List[int]]]] = defaultdict(list)
    ...
    for line in lines:
        ...
        if src_stage is not None and tgt_stage is not None and src_stage != tgt_stage:
            key = f"S{src_stage + 1}->S{tgt_stage + 1}"
            buckets[key].append((source, target, path))
    ...
    for path_type, cases in buckets.items():
        # Parse stage indices from path_type key, e.g. "S2->S4"
        pt_parts = path_type.replace("S", "").split("->")
        src_stage_idx = int(pt_parts[0]) - 1
        tgt_stage_idx = int(pt_parts[1]) - 1
        stage_gap = abs(tgt_stage_idx - src_stage_idx)
        is_composition = stage_gap > 1
        ...
        for idx, (source, target, _) in enumerate(cases):
            ...
            if len(generated_path) >= 2:
                if generated_path[0] == source and generated_path[-1] == target:
                    ...
                    if path_valid:
                        if is_composition:
                            # For Si->Sj with gap>1, check that path visits
                            # at least one node from each intermediate stage
                            ...
                            for mid in range(min_s + 1, max_s):
                                if not any(node in stage_sets[mid]
                                           for node in generated_path[1:-1]):
                                    all_intermediates_hit = False
                                    break
                            if all_intermediates_hit:
                                valid = True
                        else:
                            valid = True
...
def main() -> None:
    ...
    logger.info("Starting GraphA training")

    with open(data_dir / "stage_info.pkl", "rb") as f:
        stage_info = pickle.load(f)
    stages = stage_info["stages"]

    logger.info("Number of stages: %d", len(stages))
    ...
    for iter_num in range(args.max_iters + 1):
        ...
        if iter_num % args.test_interval == 0:
            ...
            # Sort keys by (src_stage, tgt_stage) for consistent display
            sorted_keys = sorted(
