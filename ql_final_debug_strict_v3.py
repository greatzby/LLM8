...
v3 improvements (do NOT disable KL):
1) Dynamic digit-guard constrained decoding (for ascii_digits_space_single):
   - At each step, allow only digits that keep the *current pending integer* <= node_max.
   - If appending any digit would exceed node_max (e.g., pending="61" when node_max=149),
     digits are forbidden -> only allow ' ' and EOS. This fixes 61->610 / 134->1344 failure mode.

2) Success/path semantics aligned with allow_invalid_continue:
   - Track BOTH:
       raw_path_nodes     = [source] + generated_nodes (includes invalid proposals)
       applied_path_nodes = actual state trajectory using only VALID transitions
   - Choose success mode via --success_path_mode {applied, raw}.
     Default: applied (matches "invalid transition is a no-op but episode can continue").

3) KL kept ON; default KL mode upgraded to exact masked full-distribution KL over the *dynamic*
   allowed action set (only ~12 tokens). Non-negative and lower variance than action-level estimator.

Notes:
- Dynamic digit-guard requires --action_mask ascii_digits_space_single (single-char tokens).
- If you use looser mask (multi-digit tokens), dynamic constraints are disabled automatically.
"""
...
def build_allowed_token_mask(
    ...
    mode:
      ...
      - "ascii_digits_space": legacy looser digits/spaces (any length), plus EOS
    """
    V = len(tokenizer)
...
def build_action_space_single_char(
    ...
    for tid in allowed_ids:
        ...
        if s == " ":
            ...
            # if allowed_token_mask was built for single-char mode, we shouldn't reach here
            pass
    ...
    def consume_text(self, piece: str) -> ParseResult:
        ...
        for ch in piece:
            if _is_ascii_digit(ch):
                ...
                # EARLY CUT 2: at max length, if already > node_max, invalid immediately
                if len(self.pending_digits) == self.max_digits:
                    try:
...
# -------------------------
# dynamic constrained action ids (digit guard)
# -------------------------
def _dynamic_allowed_digit_chars(
    pending: str,
    parser: NodeStreamParser,
    forbid_leading_zeros: bool,
) -> List[str]:
    # If forbidding leading zeros, once pending == "0", do not allow more digits (prevents "07" -> 7).
    ...
    for d in "0123456789":
        ...
        try:
            ...
            continue
        if v < parser.node_min or v > parser.node_max:
            continue
        out.append(d)
    return out
...
def dynamic_candidate_token_ids(
    ...
    """
    Return candidate token ids for next token, or None if dynamic constraints are disabled.
    Only supported for ascii_digits_space_single with a valid ActionSpace.
    """
    if action_space is None:
        return None
    if not args.dynamic_digit_guard:
        # static action space
    ...
    # space policy
    if args.allow_space_always or pending != "":
        parts.append(action_space.space_ids)

    # digits by guard
    digits = _dynamic_allowed_digit_chars(
        pending=pending,
        parser=parser,
    ...
    # EOS always allowed
    parts.append(action_space.eos_ids)
...
def select_next_token_from_candidates(
    ...
    """
    Sample/greedy from a subset of token ids.
    candidate_ids: [A]
    """
    # gather candidate logits
    cand_logits = logits_1d.detach().index_select(0, candidate_ids)

    # if all -inf, fallback to uniform random among candidates
    ...
    # epsilon exploration: pick random among top epsilon_explore_top_k within candidates
    if epsilon > 0.0 and random.random() < epsilon:
        K = int(epsilon_explore_top_k)
        K = max(1, min(K, candidate_ids.numel()))
        vals, idx = torch.topk(cand_logits, K)
        ridx = idx[torch.randint(0, idx.numel(), (1,), device=idx.device)]
        return int(candidate_ids[ridx].item())

    # optional top-k within candidates
    if top_k > 0:
...
def compute_pending_states_for_actions(
    tokenizer,
    actions: List[int],
    node_max: int,
    node_min: int,
    ...
    """
    pending_states[t] = parser.pending_as_str() BEFORE consuming actions[t]
    Returns length T+1, with pending_states[T] being state after consuming all actions.
    Assumes actions are single-char digits/space/EOS (best with ascii_digits_space_single).
    """
    parser = NodeStreamParser(node_max=node_max, node_min=node_min)
    pending_states: List[str] = []
    for aid in actions:
        pending_states.append(parser.pending_as_str())
        if int(aid) == int(eos_id):
            # finalize flushes pending
            _ = parser.finalize()
        else:
            piece = tokenizer.decode([aid], skip_special_tokens=False, clean_up_tokenization_spaces=False)
            pr = parser.consume_text(piece)
            if pr.invalid_char:
                # stop tracking; but still return something deterministic
                # (keep current pending; future constraints won't matter much since episode should end)
                pass
    pending_states.append(parser.pending_as_str())
...
def sample_trajectory_hf(
    ...
    generated_nodes: List[int] = []          # raw parsed nodes (includes invalid proposals)
    applied_path_nodes: List[int] = [source] # actual env transitions (valid edges only)
    ...
    def apply_node_transition(next_node: int) -> Tuple[float, bool]:
        ...
        if graph.has_edge(str(current_node), str(next_node)):
            ...
            # stage bridge shaping
            if args.reward_stage_bridge > 0.0 and required_stages:
                if int(next_stage) in required_stages:
                    if args.reward_stage_bridge_only_once:
                        if int(next_stage) not in visited_required_stages:
            ...
            # deter wrong node in final stage for adjacent-stage pairs
            if (
            ...
            applied_path_nodes.append(next_node)
            ...
            # invalid transition is a "no-op" in state, but can continue if allow_invalid_continue
    ...
    for _step in range(max_new):
        ...
        # dynamic candidate ids (digit-guard) only supported for single-char action space
        ...
        if cand_ids is not None:
            ...
            # fallback: static mask (old behavior)
            logits = last_logits.detach().clone()
            if allowed_token_mask is not None:
                logits[~allowed_token_mask] = float("-inf")
            if torch.isneginf(logits).all():
                next_id = int(torch.argmax(last_logits).item())
            else:
                if args.rollout_top_k > 0:
                    logits = top_k_filtering_1d(logits, args.rollout_top_k)
                if temperature <= 1e-6:
    ...
    raw_path_nodes = [source] + list(generated_nodes)
    if args.success_path_mode == "raw":
        path_for_success = raw_path_nodes
    else:
        path_for_success = applied_path_nodes

    success = is_valid_path_k(path_for_success, source, target, node_to_stage=node_to_stage, graph=graph)
...
def q_learning_loss_single_traj(
    ...
    seg = logits[start_idx : start_idx + T, :]  # [T, V]
    ...
    with torch.no_grad():
        ...
        target_seg = target_logits[start_idx : start_idx + T, :]  # [T,V]

        next_max = torch.zeros_like(q_selected)

        if T > 1:
            # Prefer dynamic candidate sets if available
            if action_space is not None and args.action_mask == "ascii_digits_space_single":
                eos_id = int(tokenizer.eos_token_id)
                pending_states = compute_pending_states_for_actions(
                    tokenizer=tokenizer,
                    actions=actions,
                ...
                # pending_states[t+1] corresponds to state AFTER consuming action t (i.e., before action t+1)
                for t in range(T - 1):
                    pending_next = pending_states[t + 1]
                    parser_tmp = NodeStreamParser(node_max=args.node_max, node_min=args.node_min)
                    parser_tmp.pending_digits = list(pending_next)  # inject state
                    cand_ids = dynamic_candidate_token_ids(
                    ...
                    if cand_ids is None:
                        # fallback static
                        if allowed_token_mask is not None:
                            row = target_seg[t + 1].clone()
                            row[~allowed_token_mask] = float("-inf")
                            next_max[t] = row.max()
                        else:
                            next_max[t] = target_seg[t + 1].max()
                    else:
                        next_max[t] = target_seg[t + 1].index_select(0, cand_ids).max()
            else:
                # static mask
                if allowed_token_mask is not None:
                    masked = target_seg[1:].clone()
                    masked[:, ~allowed_token_mask] = float("-inf")
                    next_max[:-1] = masked.max(dim=-1).values
                else:
...
def kl_loss_masked_full_dynamic(
    policy_model,
    ref_model,
    tokenizer,
    traj_ids: List[int],
    ...
    """
    Exact KL(pi||ref) over the *dynamic allowed candidate set* at each timestep.
    Non-negative and low-variance when action space is tiny (digits/space/eos).
    """
    x_ids = torch.tensor(traj_ids[:-1], dtype=torch.long, device=device).unsqueeze(0)
    logits_pi = forward_logits(policy_model, x_ids)[0]
    logits_ref = forward_logits(ref_model, x_ids)[0]
    ...
    for t in range(T):
        ...
        parser_tmp = NodeStreamParser(node_max=args.node_max, node_min=args.node_min)
        parser_tmp.pending_digits = list(pending)
        cand_ids = dynamic_candidate_token_ids(
            pending=pending,
            parser=parser_tmp,
            action_space=action_space,
        ...
        lp = seg_pi[t].index_select(0, cand_ids)
        lr = seg_ref[t].index_select(0, cand_ids)

        logp = F.log_softmax(lp, dim=-1)
        logq = F.log_softmax(lr, dim=-1)
        p = logp.exp()
        kl_t = (p * (logp - logq)).sum(dim=-1)  # scalar
        kls.append(kl_t)
...
def dump_traj_debug(
    ...
    raw_path_nodes = traj.get("raw_path_nodes", [])
    applied_path_nodes = traj.get("applied_path_nodes", [])
    bucket = traj.get("bucket", None)

    print("\n" + "=" * 110)
    print(f"[DEBUG][{tag}] iter={iteration} pair=({s}->{t}) bucket={bucket}")
    print(
        f"  success={traj.get('success')} hit_target={traj.get('hit_target')}"
    ...
    print(
        f"  covered_all_required={traj.get('covered_all_required')} covered_required_ratio={traj.get('covered_required_ratio')}"
    )
    if isinstance(path_nodes, list) and len(path_nodes) > 0:
        print(f"  path_len={len(path_nodes)} start={path_nodes[0]} end={path_nodes[-1]} target={t} (mode={args.success_path_mode})")
    else:
        print(f"  path_len=0 (mode={args.success_path_mode})")
    ...
    print(f"  raw_path_nodes     ({len(raw_path_nodes)}): {raw_path_nodes}")
    print(f"  applied_path_nodes ({len(applied_path_nodes)}): {applied_path_nodes}")
    if isinstance(path_nodes, list):
        print(f"  path_nodes(used)   ({len(path_nodes)}): {path_nodes}")
...
def load_policy_model_and_ref(
    ...
    def _load_base():
        kwargs = dict(dtype=torch_dtype, trust_remote_code=bool(args.trust_remote_code))
        if args.load_in_4bit:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Q-learning (HF/Qwen) for composition graphs (K stages) STRICT-SINGLE-CHAR MASK v3")
    ...
    p.add_argument(
        ...
        help=(
            "ascii_digits_space_single: allow ONLY single-char '0'..'9' or ' ' plus EOS (RECOMMENDED).\n"
            "ascii_digits_space: legacy looser (allows multi-digit tokens like '1444' - NOT recommended)."
        ),
    )

    # Dynamic digit guard (only for single-char mode)
    p.add_argument("--dynamic_digit_guard", action=argparse.BooleanOptionalAction, default=True)
    ...
    p.add_argument("--reward_invalid_token", type=float, default=3.0)
    ...
    p.add_argument("--kl_mode", choices=["action", "masked_full_dynamic"], default="masked_full_dynamic")
    ...
    p.add_argument("--log_dir", type=str, default="out_ql_hf_final_v3")

    p.add_argument("--debug_interval", type=int, default=0)
    p.add_argument("--debug_num_train", type=int, default=0)
    p.add_argument("--debug_num_eval", type=int, default=0)
    p.add_argument("--debug_only_fail", action="store_true")
    p.add_argument("--debug_show_token_pieces", action="store_true")
...
def main() -> None:
    ...
    print(
        f"[meta] block_size={block_size}, eos_id={tokenizer.eos_token_id}, pad_id={tokenizer.pad_token_id}, vocab={len(tokenizer)}"
    )
    ...
    if args.action_mask != "none":
        ...
        print(
            f"[action_mask] mode={args.action_mask} enabled. allowed={kept}/{len(tokenizer)} tokens. build_sec={time.perf_counter()-t0:.2f}"
        )
        ...
        if args.action_mask == "ascii_digits_space_single":
            ...
            # dynamic constraints not supported in loose modes
            if args.dynamic_digit_guard:
                print("[warn] dynamic_digit_guard is only supported for ascii_digits_space_single; disabling.")
                args.dynamic_digit_guard = False
    else:
    ...
    for iteration in range(1, args.max_iters + 1):
        ...
        for (s, t) in batch_pairs:
            ...
            if kl_coef_cur > 0.0 and ref_model is not None:
                if args.kl_mode == "masked_full_dynamic" and action_space is not None and args.action_mask == "ascii_digits_space_single":
                    kl = kl_loss_masked_full_dynamic(
