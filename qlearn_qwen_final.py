...
def build_allowed_token_mask_digits_whitespace(
    ...
    (保持你原逻辑，不额外收紧；如需收紧我再给你一个 switch 版。)
    """
    V = len(tokenizer)
...
def sample_trajectory_hf(
    ...
    # -------- [NEW] reward_miss_target: 结束但失败要扣分，防止模型学会立刻停 --------
...
def q_learning_loss_single_traj(
    ...
    device: torch.device,
    gamma: float,
    allowed_token_mask: Optional[Tensor] = None,  # [NEW]
    ...
    with torch.no_grad():
        ...
        # -------- [NEW] mask TD max to the same allowed action space --------
        if allowed_token_mask is not None:
...
def parse_args() -> argparse.Namespace:
    ...
    # [NEW] miss target penalty (positive number; applied as -reward_miss_target)
    ...
    for iteration in range(1, args.max_iters + 1):
        ...
        for (s, t) in batch_pairs:
            ...
            qloss, td_err, _ = q_learning_loss_single_traj(
                ...
                device=device,
                gamma=args.gamma,
                allowed_token_mask=allowed_token_mask,  # [NEW]
            )
            loss_i = qloss
            kl_val = 0.0
