...
This is a drop-in replacement for the previous Q-learning/TD script:
- Keeps the SAME environment, rollout, parsing, action masking, and reward shaping.
- Replaces TD/Q-learning update with on-policy PPO-style clipped policy gradient.
- Supports GRPO-style group-relative advantage normalization (no critic/value head needed).

Key ideas:
- For each (source, target) prompt, sample `group_size` trajectories from the current policy.
- Compute advantages either:
  * sequence: broadcast a scalar advantage per trajectory (GRPO default)
  * rtg: token-level reward-to-go advantage (better for process shaping)
- Optimize clipped surrogate objective (PPO) + KL penalty to a frozen reference model.

Recommended for on-policy correctness:
- Set epsilon exploration to 0 (disable epsilon-greedy).
- Use top_k=0 (no truncation) and a fixed temperature (often 1.0).
"""
...
def apply_node_reward_credit_to_digits(
    ...
    Returns the residual reward that should remain on the current (delimiter) token step.
    """
    if mode == "none":
...
def dynamic_candidate_token_ids(
    ...
    # No semantic guards -> optionally apply a tiny FORMAT-only mask:
    # disallow SPACE when pending is empty (removes redundant "double spaces").
    ...
    parts2: List[Tensor] = []
    parts2.append(action_space.eos_ids)  # Always allow EOS
...
def select_next_token_from_candidates(
    ...
    # NOTE: epsilon-greedy breaks on-policy assumptions. For PPO/GRPO, set epsilon=0.
...
def masked_logprob_and_entropy_from_logits(
    ...
    """
    Compute log pi(a|s) and entropy of the *masked, renormalized* policy over candidate_ids.
    Optionally applies the same temperature/top_k as used in sampling.
    """
    ...
    if pos.numel() == 0:
        # Should not happen if sampling & candidate builder are consistent.
...
def sample_trajectory_hf(
    ...
    if (not sampled_ids) or (int(sampled_ids[-1]) != int(eos_id)):
        ...
        # for EOS, we don't have cached last_logits here; just set old_logprob to 0 (neutral) if missing
        # However, this branch is rare; better to keep lengths consistent.
    ...
    success_applied = is_valid_path_k(applied_path_nodes, int(source), int(target), node_to_stage=node_to_stage, adj=adj)
...
# -------------------------
# KL loss (masked, dynamic candidates) – keep your original implementation
# -------------------------
...
def discounted_rtg(rewards: List[float], dones: List[bool], gamma: float) -> List[float]:
    """
    Reward-to-go with terminal masking: G_t = r_t + gamma * G_{t+1} * (1-done_t)
    """
    T = len(rewards)
    out = [0.0] * T
    g = 0.0
...
def compute_advantages_for_group(
    ...
    """
    Returns list of tensors, one per trajectory, shaped [T].
    """
...
def compute_new_logprobs_and_entropy_for_traj(
    tokenizer,
    logits_pi: Tensor,  # [L,V] from teacher forcing on traj_ids[:-1]
    traj_ids: List[int],
    prompt_len: int,
    actions: List[int],
    source: int,
...
def evaluate_model(
    ...
    total_correct_used = 0
    total_correct_raw = 0
    total_correct_applied = 0
    total_cases = 0
    ...
    for bname in bucket_names:
        ...
        for s, t in bpairs:
            ...
            correct_used += int(bool(traj["success"]))
            correct_raw += int(bool(traj["success_raw"]))
            correct_applied += int(bool(traj["success_applied"]))

        n = len(bpairs)
        total_cases += n
        total_correct_used += correct_used
        total_correct_raw += correct_raw
        total_correct_applied += correct_applied

        results[bname] = {
            "correct": correct_used,
            "total": n,
            "accuracy": (correct_used / n if n else 0.0),
            "accuracy_raw": (correct_raw / n if n else 0.0),
            "accuracy_applied": (correct_applied / n if n else 0.0),
        }

    results["overall"] = {
        "correct": total_correct_used,
        "total": total_cases,
        "accuracy": (total_correct_used / total_cases if total_cases else 0.0),
        "accuracy_raw": (total_correct_raw / total_cases if total_cases else 0.0),
        "accuracy_applied": (total_correct_applied / total_cases if total_cases else 0.0),
    }
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("GRPO/PPO-style (HF/Qwen) for composition graphs (K stages)")
    ...
    p.add_argument("--batch_size", type=int, default=8, help="Number of (s,t) prompts per iteration.")
    p.add_argument("--group_size", type=int, default=4, help="Trajectories per prompt (GRPO group).")
    ...
    # epsilon-greedy (NOT recommended for PPO/GRPO; keep for ablations)
    ...
    # Whether logprob computation matches rollout temp/topk (for exact on-policy ratios)
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if args.kl_coef > 0 and args.kl_anneal_iters > 0 and args.kl_min_coef <= 0:
        print("[WARN] KL is annealed with kl_min_coef<=0. This can cause late collapse. Consider --kl_anneal_iters 0.")

    if args.epsilon_start != 0.0 or args.epsilon_end != 0.0:
        print("[WARN] You enabled epsilon-greedy. This is not on-policy PPO/GRPO. Recommended: set epsilons to 0.")
...
def main() -> None:
    ...
    for iteration in range(1, args.max_iters + 1):
        ...
        # sample prompts
        batch_prompts = random.choices(train_pairs, k=args.batch_size)

        # collect rollouts grouped by prompt
        ...
        for (s, t) in batch_prompts:
            for _g in range(args.group_size):
                ...
                success_applied += int(bool(traj["success_applied"]))
        ...
        # build training loss over all trajectories
        losses: List[Tensor] = []
        kl_list: List[float] = []
        ...
        for (s, t), trajs in rollouts_by_prompt.items():
            ...
            for tr, adv in zip(trajs, advs):
                ...
                approx_kl = (old_lp - new_lp).mean()  # common PPO monitor

                loss_i = policy_loss

                # entropy bonus (masked entropy averaged over tokens)
        ...
        num_trajs = args.batch_size * args.group_size
        ...
        if args.log_interval > 0 and (iteration % args.log_interval == 0):
            record = {
                ...
                "rollout_top_k": int(args.rollout_top_k),
                "success_rate": success_used / max(1, num_trajs),
                "success_rate_raw": success_raw / max(1, num_trajs),
                "success_rate_applied": success_applied / max(1, num_trajs),
                "invalid_edge_rate": invalid_edge_count / max(1, num_trajs),
                "invalid_tok_rate": invalid_tok_count / max(1, num_trajs),
